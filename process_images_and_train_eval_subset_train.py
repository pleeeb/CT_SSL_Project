...
def create_subset_yaml(data_dir, subset_info, num_classes=1):
    """Create YOLO dataset configuration file for subset training"""
    yaml_content = {
        'path': data_dir,
        'train': subset_info['train_dir'],
        'val': 'images/val',
        'test': 'images/test',
        'nc': num_classes,
        'names': ['nodule']
    ...
    yaml_path = os.path.join(data_dir, f'luna16_subset_{subset_info["percentage"]}.yaml')
...
def create_data_subset(manifest, percentage, output_dir, seed=42):
    """Create a subset of the training data with specified percentage"""
    if percentage == 100:
        # Use full dataset
        return {
            'percentage': 100,
            ...
            'selected_images': manifest['total_images'],
            'positive_images': manifest['positive_images'],
            'negative_images': manifest['negative_images']
        }
    ...
    positive_samples = [m for m in manifest['image_metadata'] if m['has_nodule']]
    negative_samples = [m for m in manifest['image_metadata'] if not m['has_nodule']]

    # Calculate how many samples to select
    n_positive = int(len(positive_samples) * percentage / 100)
    n_negative = int(len(negative_samples) * percentage / 100)

    # Ensure we have at least some samples
    n_positive = max(1, n_positive)
    n_negative = max(1, n_negative)
    ...
    subset_dir = os.path.join(output_dir, f'subset_{percentage}')
    subset_images_dir = os.path.join(subset_dir, 'images', 'train')
    subset_labels_dir = os.path.join(subset_dir, 'labels', 'train')
    ...
    logger.info(f"Creating {percentage}% subset with {len(selected_samples)} samples")

    for sample in tqdm(selected_samples, desc=f"Copying files for {percentage}% subset"):
        # Copy image
        src_img = sample['image_path']
        dst_img = os.path.join(subset_images_dir, os.path.basename(src_img))
        shutil.copy2(src_img, dst_img)

        # Copy label
        src_label = sample['label_path']
        dst_label = os.path.join(subset_labels_dir, os.path.basename(src_label))
        shutil.copy2(src_label, dst_label)

    # Create subset info
    subset_info = {
        'percentage': percentage,
        'train_dir': os.path.join(f'subset_{percentage}', 'images', 'train'),
        'total_images': len(selected_samples),
        'selected_images': len(selected_samples),
        'positive_images': len(selected_positive),
        'negative_images': len(selected_negative),
        'seed': seed
    }

    # Save subset manifest
    subset_manifest = {
        'original_manifest': manifest['split_name'],
        'percentage': percentage,
        'seed': seed,
        'selected_samples': [s['image_path'] for s in selected_samples],
        'subset_info': subset_info
    }

    manifest_path = os.path.join(subset_dir, 'subset_manifest.json')
...
def train_yolov5(data_yaml, fold_num, output_dir, epochs=50, batch_size=16, img_size=512,
                 pretrained_weights='', data_percentage=100):
    """Train YOLOv5s model with optional data percentage"""
    import subprocess
    import gc
    import torch
    ...
    # Training command
    project_dir = os.path.join(output_dir, f'fold_{fold_num}_p{data_percentage}')
    cmd = [
        ...
        '--project', project_dir,
        '--name', f'luna16_nodule_detection_p{data_percentage}',
        '--save-period', '10',
        '--patience', '30',
        '--multi-scale',
        '--sync-bn',
    ...
    logger.info(f"Training fold {fold_num} with {data_percentage}% of data")
    ...
    best_weights = os.path.join(project_dir, f'luna16_nodule_detection_p{data_percentage}', 'weights', 'best.pt')
    return best_weights
...
def evaluate_yolov5(weights_path, data_yaml, fold_num, output_dir, manifest, img_size=512, data_percentage=100):
    """Evaluate YOLOv5 model with checkpoint support"""
    import subprocess

    yolov5_dir = os.path.join(os.path.dirname(output_dir), 'yolov5')
    project_dir = os.path.join(output_dir, f'fold_{fold_num}_p{data_percentage}_eval')
...
def run_cross_validation(base_path, annotations_path, num_folds=10, force_reload=False,
                         ...
                         data_percentage=100, seed=42):
    """Run 10-fold cross-validation with YOLOv5 using specified percentage of training data"""
    import gc
    import torch
    ...
    # Create output directory with experiment name and data percentage
    if experiment_name == "default":
        output_dir = os.path.join(base_path, f'yolov5_output_p{data_percentage}')
    else:
        output_dir = os.path.join(base_path, f'yolov5_output_{experiment_name}_p{data_percentage}')
    ...
    for test_subset in range(num_folds):
        ...
        logger.info(f"Training data percentage: {data_percentage}%")
        ...
        # Load manifests
        if not os.path.exists(train_manifest_file) or not os.path.exists(val_manifest_file):
            logger.error(f"Missing manifest files for fold {test_subset}")
            continue
        ...
        # Create subset of training data if percentage < 100
        if data_percentage < 100:
            subset_info = create_data_subset(train_manifest, data_percentage, fold_dir, seed=seed)
            data_yaml = create_subset_yaml(fold_dir, subset_info)
            logger.info(f"Created {data_percentage}% subset with {subset_info['selected_images']} images "
                       f"({subset_info['positive_images']} positive, {subset_info['negative_images']} negative)")
        else:
            # Use full dataset
            ...
            subset_info = {
                ...
                'selected_images': train_manifest['total_images'],
                'positive_images': train_manifest['positive_images'],
                'negative_images': train_manifest['negative_images']
            }

        if not os.path.exists(data_yaml):
            logger.error(f"Missing YAML file at {data_yaml}")
            continue

        # Check if model already exists
        weights_path = os.path.join(output_dir, f'fold_{test_subset}_p{data_percentage}',
                                    f'luna16_nodule_detection_p{data_percentage}', 'weights', 'best.pt')
        ...
        backup_weights = os.path.join(backup_dir, f'best_p{data_percentage}.pt')
        ...
        try:
            froc_results = evaluate_yolov5(weights_path, data_yaml, test_subset, output_dir,
                                         val_manifest, data_percentage=data_percentage)
...
def compare_data_percentages(base_path, annotations_path, experiment_name, pretrained_weights,
                             percentages=[10, 25, 50, 75, 100], num_folds=10, force_reload=False):
    """Run experiments with different data percentages and compare results"""
    comparison_results = {}

    for percentage in percentages:
        logger.info(f"Running experiment with {percentage}% of training data")
        ...
        comparison_results[percentage] = results

    # Save comparison results
    output_dir = os.path.join(base_path, f'comparison_{experiment_name}')
    os.makedirs(output_dir, exist_ok=True)

    comparison_file = os.path.join(output_dir, 'data_percentage_comparison.json')
    with open(comparison_file, 'w') as f:
        json.dump(comparison_results, f, indent=2)

    # Generate comparison report
    generate_comparison_report(comparison_results, output_dir, experiment_name)

    return comparison_results


def generate_comparison_report(comparison_results, output_dir, experiment_name):
    """Generate a report comparing results across different data percentages"""
    report_lines = []
    report_lines.append(f"Data Efficiency Comparison Report")
    ...
    report_lines.append("-" * 80)
    report_lines.append(f"{'Data %':>10} | {'Avg FROC':>10} | {'Sens@0.125':>12} | {'Sens@1':>10} | {'Sens@4':>10}")
    report_lines.append("-" * 80)

    for percentage in sorted(comparison_results.keys()):
        results = comparison_results[percentage]
        if results:
            avg_froc = np.mean([r['froc_results']['average_sensitivity'] for r in results])
            avg_sens_0125 = np.mean([r['froc_results']['sensitivity_at_0.125_fp'] for r in results])
            avg_sens_1 = np.mean([r['froc_results']['sensitivity_at_1_fp'] for r in results])
            avg_sens_4 = np.mean([r['froc_results']['sensitivity_at_4_fp'] for r in results])

            report_lines.append(f"{percentage:>10}% | {avg_froc:>10.2%} | {avg_sens_0125:>12.2%} | "
                                f"{avg_sens_1:>10.2%} | {avg_sens_4:>10.2%}")

    report_lines.append("-" * 80)
    report_lines.append("")

    # Detailed fold results
    report_lines.append("Detailed Fold Results:")
    report_lines.append("=" * 80)
    ...
    for percentage in sorted(comparison_results.keys()):
        ...
        for fold_result in results:
            ...
            subset_info = fold_result.get('subset_info', {})

            report_lines.append(f"  Fold {fold}:")
            report_lines.append(f"    - Training images: {subset_info.get('selected_images', 'N/A')} "
                                f"({subset_info.get('positive_images', 'N/A')} positive, "
                                f"{subset_info.get('negative_images', 'N/A')} negative)")
            report_lines.append(f"    - Average FROC: {froc['average_sensitivity']:.2%}")
            report_lines.append(f"    - Sensitivity @ 0.125 FP/scan: {froc['sensitivity_at_0.125_fp']:.2%}")
            report_lines.append(f"    - Sensitivity @ 1 FP/scan: {froc['sensitivity_at_1_fp']:.2%}")
            report_lines.append(f"    - Sensitivity @ 4 FP/scan: {froc['sensitivity_at_4_fp']:.2%}")
...
def main():
    ...
    data_percentage = 100 # Percentages to test
    ...
    # Print summary
    print(f"\nCross-Validation Results Summary ({data_percentage}% data)")
    ...
    for fold_result in results:
        ...
        print(f"  - Training images used: {subset_info.get('selected_images', 'N/A')}")
    ...
    """comparison_results = compare_data_percentages(
        base_path=base_path,
        annotations_path=annotations_path,
        experiment_name=experiment_name,
        pretrained_weights='',
